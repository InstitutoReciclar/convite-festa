...
export default function QrScanner({ onResult, onError }: QrScannerProps) {
  ...
  useEffect(() => {
    ...
    // Append the scanner element to the parent element
    const parentElement = document.querySelector(".aspect-square")
    if (parentElement) {
      parentElement.appendChild(scannerElement)
    }
    ...
    html5QrCode
      .start(
        ...
        (errorMessage) => {
          // On error (this is called continuously, so we don't want to report every error)
    ...
    return () => {
      ...
      if (parentElement && scannerElement) {
        parentElement.removeChild(scannerElement)
      }
    }
  }, [onResult, onError])
